...
(function () {
  ...
  mount.innerHTML =
    '<div class="dn-band">' +
      ...
      '</div>' +
      '<button type="button" class="dn-cta" data-dn-open>Support Sibility</button>' +
    '</div>';
  ...
  function formHTML() {
    ...
    return '' +
      ...
      '<div class="dn-freq">' +
        '<button type="button" class="' + seg(state.freq === 'once') + '" data-freq="once">Once</button>' +
      ...
      '<div class="dn-custom">' +
        ...
        '<input type="text" inputmode="numeric" value="' + (state.custom || '') + '" placeholder="Any sum" data-dn-custom />' +
      '</div>' +
      '<button type="button" class="dn-submit" data-dn-submit>' + label + '</button>' +
      (state.err ? '<div role="alert" class="dn-err">' + state.err + '</div>' : '') +
      '<p class="dn-note">All payments are processed securely by our payment provider.</p>';
  }
